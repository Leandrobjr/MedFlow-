            ...
            {group.items.map((item) => (
              <Link
                ...
                className={`flex items-center px-4 py-2 text-sm rounded-lg transition-colors ${
                  isMenuItemActive(item.href) ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 bg-white border-r border-gray-200 flex-col shrink-0">
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          {menuStructure.map((item) => (item.type === 'item' ? renderMenuItem(item) : renderMenuGroup(item)))}
        </nav>
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="flex items-center px-4 py-3 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 truncate">{user?.name || 'Carregando...'}</p>
              <p className="text-xs text-gray-500 truncate">{user?.role || 'Acessando...'}</p>
            </div>
          </div>
          <button
            onClick={logout}
      ...
      {isSidebarOpen && (
        <div className="md:hidden fixed inset-0 bg-black/50 z-40 backdrop-blur-sm" onClick={() => setIsSidebarOpen(false)} />
      )}

      {/* Mobile Sidebar */}
      <aside
        className={`md:hidden fixed inset-y-0 left-0 w-72 bg-white z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${
          ...
          {menuStructure.map((item) =>
            item.type === 'item' ? (
              <Link
                ...
                className={`flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-colors ${
                  isMenuItemActive(item.href) ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'
                }`}
              >
                <item.icon className="h-5 w-5 mr-3" />
                {item.label}
              ...
              <div key={item.key}>
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-3 text-sm font-medium rounded-xl transition-colors ${
                    isMenuGroupActive(item) ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'
                  }`}
                >
                  <div className="flex items-center">
                    <item.icon className="h-5 w-5 mr-3" />
                  ...
                  {expandedMenuGroups.has(item.key) ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                </button>
                  ...
                  <div className="ml-4 mt-1 space-y-1 border-l-2 border-gray-200 pl-2">
                    {item.items.map((subItem) => (
                      <Link
                        ...
                        onClick={() => setIsSidebarOpen(false)}
                        className={`flex items-center px-4 py-2 text-sm rounded-lg transition-colors ${
                          isMenuItemActive(subItem.href) ? 'bg-blue-50 text-blue-600 font-medium' : 'text-gray-600 hover:bg-gray-50'
                        }`}
                      >
