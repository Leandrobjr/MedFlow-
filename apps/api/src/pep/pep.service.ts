...
export class PepService {
  ...
  async create(
    ...
    auditContext?: { userId: string; ip?: string; ua?: string },
  ) {
    const record = await this.prisma.withTenant(tenantId, async (tx) => {
      ...
      if (existing) {
        throw new BadRequestException(
          'Já existe um prontuário para este atendimento.',
        );
      }
  ...
  async findByPatient(tenantId: string, patientId: string) {
    return this.prisma.withTenant(tenantId, async (tx) => {
      return tx.medicalRecord.findMany({
  ...
  async update(
    ...
    console.log('[PepService.update] Iniciando atualização:', {
      tenantId,
      id,
      dto,
    });
    ...
    if (updated.isFinalized && updated.appointmentId && !record.isFinalized) {
      console.log(
        `[PepService.update] ⚠️ Prontuário foi finalizado via update. Tentando criar repasse retroativamente para appointment: ${updated.appointmentId}`,
      );
      try {
        await this.financeService.createMedicalFeeRetroactively(
          tenantId,
          updated.appointmentId,
        );
        console.log(
          `[PepService.update] ✅ Repasse criado com sucesso após finalização via update.`,
        );
      } catch (error) {
        console.error(
          `[PepService.update] ❌ ERRO ao criar repasse retroativamente após update:`,
          error,
        );
  ...
  async finalize(
    ...
    if (record.isFinalized) {
      if (record.appointmentId) {
        try {
          console.log(
            `[PepService.finalize] Prontuário já finalizado. Verificando se precisa criar repasse retroativamente para appointment: ${record.appointmentId}`,
          );
          await this.financeService.createMedicalFeeRetroactively(
            tenantId,
          ...
          console.error(
            `[PepService.finalize] Erro ao criar repasse retroativamente:`,
            error,
          );
        }
      }
    ...
    if (record.appointmentId) {
      try {
        console.log(
          `[PepService.finalize] ✅ Prontuário finalizado. Tentando criar repasse retroativamente para appointment: ${record.appointmentId}`,
        );
        ...
        console.log(
          `[PepService.finalize] ✅ Método createMedicalFeeRetroactively concluído sem erros.`,
        );
      } catch (error) {
        ...
        console.error(
          `[PepService.finalize] ❌ Stack trace:`,
  ...
  async addAddendum(
    ...
    const addendum = await this.prisma.withTenant(tenantId, async (tx) => {
      return tx.medicalAddendum.create({
        data: {
