...
export class PrismaService implements OnModuleInit, OnModuleDestroy {
  ...
  get client(): PrismaClient {
    if (!this._client) {
      ...
      if (!databaseUrl) {
        this.logger.error(
          `[DEBUG] ❌ DATABASE_URL não está definida nas variáveis de ambiente!`,
        );
        throw new Error(
      ...
      this.logger.log(
        `[DEBUG] Criando PrismaClient com DATABASE_URL: ${databaseUrl.replace(/:[^:@]+@/, ':****@')}`,
      );
  ...
  // Acesso direto aos delegates do PrismaClient (ex.: this.prisma.user.*)
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      `[DEBUG] DATABASE_URL configurada: ${databaseUrl.replace(/:[^:@]+@/, ':****@')}`,
    );
    this.logger.log(`[DEBUG] ========================================`);
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.logger.log(
          `[DEBUG] Tentativa de conexão ${attempt}/${maxRetries}...`,
        );
        ...
        this.logger.log(`[DEBUG] ========================================`);
        this.logger.log(
          `[DEBUG] ✅ Conexão estabelecida com sucesso na tentativa ${attempt}`,
        );
        this.logger.log(`[DEBUG] ========================================`);
        return;
      } catch (error: any) {
        ...
        this.logger.error(
          `[DEBUG] ❌ Falha na tentativa ${attempt}/${maxRetries}`,
        );
        ...
        if (attempt < maxRetries) {
          ...
          this.logger.log(
            `[DEBUG] Aguardando ${retryDelay}ms antes da próxima tentativa...`,
          );
          await new Promise((resolve) => setTimeout(resolve, retryDelay));
        } else {
          this.logger.error(`[DEBUG] ========================================`);
          this.logger.error(
            `[DEBUG] ❌ Todas as ${maxRetries} tentativas falharam!`,
          );
   ...
   *             Pode retornar Promise<T> ou T diretamente
  ...
  async withTenant<T>(
    ...
    return this.client.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const validationResult = await tx.$queryRaw<
        Array<{ current_setting: string | null }>
      >`
        SELECT current_setting('medflow.current_tenant', true) as current_setting
      `;      const currentTenantId = validationResult[0]?.current_setting;      if (!currentTenantId || currentTenantId !== tenantId) {
      ...
      // A função pode retornar Promise<T> ou T diretamente, então usamos Promise.resolve para normalizar
